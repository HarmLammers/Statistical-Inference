...
```{example uniform, eval=TRUE, results = 'markup'}
hist(runif(1000))
...
We need to do 1000 simulations in which we derive a sample mean out of 40 simulated values for an exponential distribution with rate 0.2.
...
g <- ggplot(data.frame(x = 1:n_sim, y = means_cum), aes(x = x, y = y))
...
print(sprintf("The sample means converges with increasing iterations to: %f", means_cum[n_sim]))
print(sprintf("Which gets close to the theoretical mean of the exponential distribution (1/lambda with lambda = 0.2): %f", mu))
...
#Report the computed numbers
print(sprintf("The theoretical variance of the exponential distribution is (1/lambda)^2: %f", (1/lambda)^2))
print(sprintf("The estimated variance of the distribution is n * variance(means): %f", n * var(means_sim)))
```
...
We can illustrate this with plots of the standard normal distribution (red) overlayed by the distribution of the means with increasing number of simulations. This report only contains the plots for the original value (n_sim = 1000) and (n_sim = 30.000)
...
As you can see, when the number of simulations increases, the fit seems to be better and the estimated distribution follows the normal distribution closely.
...
```{ToothGrowth - Structure of the experiment}
...
So we have a 95% confidence interval that does not contain 0, telling us to believe that supp. OJ performs statistically significantly better than supp. VC with dose 0.50.
...
So we have a 95% confidence interval that does not contain 0, telling us to believe that supp. OJ performs statistically significantly better than supp. VC with dose 1.0.
...
So we have a 95% confidence interval that does contain 0, telling us to believe that supp. OJ and supp. VC perform statistically equally with dose 2.0.
...
So we have 95% confidence intervals that do not contain 0, telling us to believe that doubling the dose with supp. OJ performs statistically significantly better starting with dose 0.50 up to dose 2.0.
...
So we have 95% confidence intervals that do not contain 0, telling us to believe that doubling the dose with supp. VC performs statistically significantly better starting with dose 0.50 up to dose 2.0.


##Conclusions and Assumptions
###Conclusions
- Guinea pigs given the OJ supplement at 0.5 and 1.0 dosages have significantly faster tooth growth than guinea pigs given VC at the same doses.
- Guinea pigs given OJ or VC at a dose of 2.0 do not have significantly different tooth growth
